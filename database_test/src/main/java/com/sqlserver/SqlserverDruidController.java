...
@Controller
public class SqlserverDruidController {
    @Resource
    private DataSource druidDSSqlserver;
    ...
    @RequestMapping("/sqlserverDruid")
    @ResponseBody
    public String test(HttpServletResponse resp) throws IOException {
        resp.setContentType("text/html;charset=UTF-8");
        StringBuffer stringBuffer = new StringBuffer();
        stringBuffer.append("<html><head><title></title></head><body><h2>Sqlserver Druid</h2>");
        String queryResult = "<a style=\"color: red\">norecord</a>";

        try {
            Connection conn = druidDSSqlserver.getConnection();
